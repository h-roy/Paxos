...
kill_acceptor_at_instance = None # or set it to n if you want to kill acceptors at instance n. If this is None acceptors are not killed.
kill_acceptor_id = [3]
		...
		if phase == '1B':
			...
			if self.leader == True:
				if content != 'FAIL!':
					...
					if self.states[instance]['quorum1B'] == math.ceil((num_acceptors_global+1) / 2):
						# print('quorum 1B')
			...
			if self.leader == True:
				...
				if content != 'FAIL!':
					...
					if self.states[instance]['quorum2B'] == math.ceil((num_acceptors_global+1) / 2):
						...
						s.sendto(message_3, config['proposers'])
				else:
					self.states[instance]['c-rnd'] += self.max_proposers
					# print(self.states[instance]['c-rnd'])
					message_1A = message_class(instance = instance ,phase = '1A' , content = {'c-rnd': self.states[instance]['c-rnd']})
		...
		if phase == '3':
			if instance not in self.states:
				self.states[instance] = {'c-rnd': self.id, 'c-val': None,
											  ...
											  'v-val': 0, 'quorum1B': 0,
											  'quorum2B': 0}
			self.states[instance]['c-rnd'] = content['c-rnd']
			self.states[instance]['c-val'] = content['c-val']
			self.states[instance]['decided'] = True
...
class acceptor_class(Thread):
	def __init__(self, config, id):
		...
		self.id = id
		self.instance_number = 0
		global kill_acceptor_at_instance
		global kill_acceptor_id
		global num_acceptors_global
	...
	def run(self):
		...
		while True:
			...
			if msg.message['instance'] == kill_acceptor_at_instance and id in kill_acceptor_id:
				print('-------------------killing acceptor {} at instance {}--------------------'.format(id, msg.message['instance']))
				kill_acceptor_id.remove(id)
				num_acceptors_global = 2
